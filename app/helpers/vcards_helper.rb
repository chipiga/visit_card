...
module VcardsHelper
  module FormBuilder
    ...
    #   output = "<fieldset class='#{method.to_s.downcase}'><legend>#{method.to_s.humanize}</legend>"
    #   collection.each do |value|
    #     output << check_box_collection_check_box(method, value) << "\n"
    #     output << label(method, value.humanize, :value => value.downcase) << "\n"
    #   end
    #   output << check_box_collection_hidden_field(method) << '</fieldset>'
    #   output.html_safe
    # end
    # 
    # def check_box_collection2(method, collection, options = {})
    #   output = "<fieldset class='#{method.to_s.downcase}'><legend>#{method.to_s.humanize}</legend>"
    #   collection.each do |key, value|
    #     output << check_box_collection_check_box2(method, key, :id => "#{object_name}_#{method}_#{value}") << "\n"
    #     output << label(method, value.humanize, :value => value.downcase) << "\n"
    #   end
    #   output << '</fieldset>'
    #   output.html_safe
    ...
    # def check_box_collection_check_box(method, value, options = {})
    #   ActionView::Helpers::InstanceTag.new(object_name, method, self, options.delete(:object)).
    #                                    to_check_box_collection_check_box_tag("#{object_name}[#{method}][]", value,
    #                                                                           object.send(method.to_sym).include?(value), options)
    # end
    # 
    # def check_box_collection_check_box2(method, value, options = {})
    #   ActionView::Helpers::InstanceTag.new(object_name, method, self, options.delete(:object)).
    #                                    to_check_box_collection_check_box_tag("#{object_name}[#{method.to_s.singularize}_ids][]", value,
    #                                                                           object.send(:"#{method.to_s.singularize}_ids").include?(value), options)
    # end
